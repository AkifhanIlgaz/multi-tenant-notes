...
func SeedDatabase(dbMux *repositories.DBMultiplexer) error {
...
func ResetAndSeed(dbMux *repositories.DBMultiplexer) error {
	if err := DropTables(dbMux); err != nil {
		return fmt.Errorf("failed to drop tables: %w", err)
	}

	if err := RunMigrations(dbMux); err != nil {
		return fmt.Errorf("failed to run migrations: %w", err)
	}

	if err := SeedDatabase(dbMux); err != nil {
